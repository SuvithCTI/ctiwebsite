    ...
    <div className="pt-32 pb-24 min-h-screen bg-gradient-to-br from-[#FFFDF9] via-[#FAF6F0] to-[#FFF8F2] text-[#050B14] relative overflow-hidden select-none">
      ...
      <div className="max-w-[1440px] mx-auto px-3.5 sm:px-6 lg:px-8 relative z-10 space-y-6 sm:space-y-8">
        ...
        <div className="space-y-2.5 sm:space-y-3">
          <h1 className="font-editorial text-2xl sm:text-4xl lg:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#EC4899] via-[#9333EA] to-[#0284C7] tracking-tight">
            Our Portfolio & Live Work
          </h1>
          <p className="max-w-3xl text-xs sm:text-base font-semibold leading-relaxed text-slate-700">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3.5 sm:gap-4 pt-1">
          ...
          {/* Category Filter Pills - Standalone Floating Pills with Pink Active State */}
          <div className="-mx-3.5 px-3.5 sm:mx-0 sm:px-0 overflow-x-auto no-scrollbar py-1">
            <div className="flex items-center gap-2 sm:gap-2.5 w-max">
              {categories.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`category-pill-active relative flex items-center gap-2 px-4 py-2 sm:px-5 sm:py-2.5 rounded-full text-xs font-black transition-all duration-200 cursor-pointer whitespace-nowrap select-none active:scale-95 ${
                      isSelected
                        ? 'bg-gradient-to-r from-[#FF2A6D] to-[#FF4D4D] text-white shadow-md shadow-pink-500/25 border-0'
                        : 'bg-white text-slate-700 hover:text-slate-950 hover:bg-slate-50 border border-slate-200/90 shadow-xs'
                    }`}
                    style={{
          ...
          {/* Right Aligned Modern Search Bar with Pink Accent */}
          <div className="relative w-full lg:w-84 shrink-0">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#FF2A6D] pointer-events-none" />
            <input
              type="text"
              placeholder="Search tech, industry or keyword..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-11 pr-20 py-2.5 sm:py-3 rounded-full bg-white border border-pink-200/90 text-xs font-bold text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-pink-300/30 focus:border-[#FF2A6D] shadow-xs transition-all"
            />
            {searchQuery && (
              <button
                type="button"
